...
export default function DragStage({ onComplete }: Props) {
  ...
  const [gameStep, setGameStep] = useState<'start' | 'playing' | 'celebrating' | 'valentine'>('start');
    ...
    <BackgroundLayout>
      <div className="drag-area" ref={constraintsRef}>
        ...
        <div className="ui-overlay">
          {gameStep === 'start' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="step-ui">
              <h2 className="nub-text">Hungry Nub</h2>
              <button className="play-btn" onClick={() => setGameStep('playing')}>Play</button>
            </motion.div>
          )}
          ...
          {gameStep === 'celebrating' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="step-ui">
              <h2 className="congrat-text">Congrat! You found out what nub want to eat the most!</h2>
            </motion.div>
          )}

          {gameStep === 'valentine' && (
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="step-ui">
              <h2 className="valentine-text">Will you be the pink Nub for my Valentine?</h2>
              <div className="button-group">
                <button className="btn yes-btn" onClick={onComplete}>Yes</button>
                <AnimatePresence>
                  {noButtonScale > 0 && (
                    <motion.button 
        ...
        <AnimatePresence>
          {gameStep === 'playing' && items.map((item) => !item.fed && (
            <motion.img
              key={item.id}
              src={item.img}
              drag
